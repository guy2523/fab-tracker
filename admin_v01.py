...
from notion.notion_ops import update_page_properties, create_measure_page, set_relation, update_date_range, archive_page, get_page, create_fab_page, get_page_url_by_title, get_notion_client, get_cooldown_page
...
if mode == "Delete Run":
    ...
    with st.form("delete_run_form"):
        ...
        # -------------------------------
            ...
            # ----------------------------------------
            # Fetch run info (safe here — survives rerun)
            # ----------------------------------------
            snap = firestore_get("runs", doc_id, id_token)
            fields = snap.get("fields", {}) if snap else {}
            ...
            # ----------------------------------------
            # Enhanced warning (THIS is what user sees)
            # ----------------------------------------
            st.warning(
            ...
            with stage_tab:
                ...
                with sub_notion:
                    if target_meta == "design":
                        ...
                        # Always pull the CURRENT metadata from session
                        meta = st.session_state.get("update_meta") or {}
                        design_meta = list(meta.get("design") or [])
                            ...
                            if st.button("Link", key=f"btn_design_notion_{loaded_run_doc_id}"):
                                ...
                                if existing_url:
                                    ...
                                    if not page_url:
                                        ...
                                        st.success("Design Notion page linked.")
                                        st.rerun()
                        ...
                        fabin = (_kv_get(fab_list, "FABIN") or "1970-01-01").strip()
